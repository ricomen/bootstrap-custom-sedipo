...
| 13 | Шрифт бейджа | в Colors не указан | Inter Regular (наследуется от body) | `Manrope Bold 700` | Бейдж требует отдельную гарнитуру, отличную от body (`Inter`) | Добавлен `$token-font-family-display: "Manrope", …`; используется только в `.badge`. Пакет шрифта **пока не подключён** — работает фолбэк на Inter |
...
6. **Шрифт `Manrope` для бейджа** — это финальное решение? Если да: подключить пакет (`@fontsource/manrope` или Google Fonts) и подтвердить начертание (сейчас в Figma — Bold 700). Если нет — оставить наследование от body (`Inter`).
